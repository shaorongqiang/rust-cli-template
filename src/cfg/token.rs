...
const TOKEN_ACCESS_KEY_PATH: &str = "access_key.pem";
const TOKEN_ACCESS_TOKEN_EXPIRED_MINUTES: &str = "10";
const TOKEN_REFRESH_KEY_PATH: &str = "refresh_key.pem";
...
impl TokenConfig {
    ...
    pub(super) fn resolve_paths(&mut self, cfg_dir: &Path) {
        if !self.access_key_path.is_absolute() {
            self.access_key_path = cfg_dir.join(&self.access_key_path);
        }

        if !self.refresh_key_path.is_absolute() {
